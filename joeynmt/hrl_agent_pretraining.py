...
"""
A sligthly modified clone of hrl_agent that supports minibatching
(used exclusively for pretraining)
"""
...
from joeynmt.agent import HRLBaseAgent as Agent
...
class HighLevelAgent(Agent):
    ...
    def forward(self, obs):
        ...
        if subtask_id is None:
            ...
            scores = torch.where(self.completed_subtasks,
                                 scores.new_full([1], float('-inf')), scores).to(self.device)
    ...
    def state(self):
        ...
        """
        #print([agent.device for agent in self.low_level_agents])
        #print('Lowlevel', [agent.state.device.index for agent in self.low_level_agents])
        #print(self.device)
        #print(self.completed_subtasks.device)
...
class LowLevelAgent(Agent):
    ...
    def forward(self, obs, states):
        # text = obs[0].to(self.device)  # User answer (may be empty)
        ...
        if not self.start_episode and isinstance(text[0], torch.Tensor):
            self.current_user_answer = text
            user_answer_encoding = self.user_understanding_module(
                self.current_user_answer)

        # If answers received:
        if bool(len(self.current_user_answer)):
            #print(self.current_recipe_encoding)
            #print(user_answer_encoding)
            v_i = (
                1 - self.w_d
...
class HierarchicalAgent(nn.Module):
    ...
    def __init__(self, agents: List[Agent], env: HRLEnvironment, device: torch.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')):
        """
        ...
        self.high_level_agent = HighLevelAgent(self.low_level_agents, device=device).to(device)
    ...
    def id_state_dict(self) -> dict:
        ...
        :returns: id - model
        """
        d = {4: self.high_level_agent.input_dim}
        for low_lvl_agent_id in list(self.agent_id.values())[:-1]:
            d[low_lvl_agent_id] = self.low_level_agents[
                low_lvl_agent_id].hidden_dim
        return d

    @property
    def id_action_dict(self) -> dict:
        d = {4: self.high_level_agent.output_dim}
        for low_lvl_agent_id in list(self.agent_id.values())[:-1]:
            d[low_lvl_agent_id] = self.low_level_agents[
                low_lvl_agent_id].output_dim
        return d
    ...
    def identity(self) -> int:
        ...
        if self.high_level_agent.acting:
            return self.agent_id['high_level_agent']
        else:
            return self.agent_id[str(self.low_level_agents[
...
def build_agent(
        ...
        env: HRLEnvironment,
        warmstart: bool = True,
        pretrained_path: str = f'{os.getcwd()}/models/') -> HierarchicalAgent:
    ...
    for i in range(num_subtasks):
        ...
        low_level_agents.append(
            LowLevelAgent(
                recipe_understanding_module=recipe_encoder,
                user_understanding_module=answer_encoder,
                hidden_dim=hidden_size,
                output_dim=env.low_level_action_space[i].n).to(device)
            )
    ...
    if warmstart:
        ...
        for i, low_level_agent in enumerate(low_level_agents):
            ...
            low_level_agent.load_state_dict(torch.load(path, map_location=device))
            low_level_agent.train()

    return HierarchicalAgent(low_level_agents, env, device=device).to(device)
