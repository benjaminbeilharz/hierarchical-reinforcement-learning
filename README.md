## Check the original repository: [https://github.com/joeynmt/joeynmt](joeynmt)
...
JoeyNMT is now also capable of performing Reinforcement Learning 
with integration of OpenAI Gym. This allows an easy implementation of both 
Classic Control reinforcement 
learning scenarios such as [Acrobot](https://gym.openai.com/envs/Acrobot-v1/) 
and [Cartpole](https://gym.openai.com/envs/CartPole-v1/).
To load and train these environments, follow the example in
`configs/acrobot.yaml`.
1. Change the `env_name` variable to the OpenAI Gym environment of choice
2. Set the `scenario` variable accordingly. Choices: `"ClassicControl"`, 
    `"Atari"`, and `"HRL"` 
...
- **Custom Environment**: Inherit the OpenAI Gym class and follow the [OpenAI Gym 
    API](https://gym.openai.com/docs/#observations)
- **Custom Agent**: Implement the abstract interface defined in 
    `joeynmt/agent.py` and override the defined methods therein
- **Actor-Critic methods**: Implement value networks following the abstract 
    interface in `joeynmt/value_network.py` 

### Hierarchical reinforcement learning tasks
Furthermore, JoeyNMT supports hierarchical reinforcement learning tasks such as 
Interactive Semantic Parsing](https://github.com/LittleYUYU/Interactive-Semantic-Parsing) 
by Yao et al., as is described in the following sections.

#### Task description
The implemented hierarchical reinforcement approach is an effort to reimplement
the [Interactive Semantic
Parsing](https://github.com/LittleYUYU/Interactive-Semantic-Parsing) approach
by Yao et al. The supervised baselines have been re-implemented, the reinforcement
learning method, and replaced the Latent Attention Model with JoeyNMT\'s
Transformer Encoder. 

The input is a recipe description - too see what they look like please have a
look at our data file (label: words). The model asks the user follow-up
questions if the initial parse of the recipe description is not sufficient for a
confident prediction.  To reduce the number of questions, Reinforcement Learning
is employed. This approach is considered **hierarchical** because the queries
are subdivided into four subtasks which gives the model the opportunity to
optimise the order in which it processes the subtasks and minimise user
interaction.
...
---
In the case of the described hierarchical reinforcement learning approach
described above:
To pretain the low-level agents, run 

```bash
python ./subtask_pretraining_enhanced.py
```

If you want to customize the pretraining, you can change the parameters from
**line 66 to 74** - do not change `SEED` and  `ASK_LABELS`.
...
To customize the training process, the configuration files can be altered or you
can write your own. If you decide to write your own, please have look at the
files in the `configs` directory.
The comments in the configuration files will tell you which options you have for
each parameter, what these options mean, and what the default settings are if
available.
...
The evaluation output will be printed to screen. To save this output to a file,
add the `--output_file <output_path>` option to the testing and evaluating
command above.
